    ...
    pub fn run(
        ...
        for mut tokens in token_pairs.clone() {
            ...
            let ibc_denom_a = ctx
                ...
                .ok_or(localic_utils::error::Error::MissingContextVariable(
                    format!("ibc_denom::{}", &token_a),
                ))?;
            let ibc_denom_b = ctx
                .get_ibc_denom(&token_b, NEUTRON_CHAIN_NAME, OSMOSIS_CHAIN_NAME)
                .ok_or(localic_utils::error::Error::MissingContextVariable(
                    format!("ibc_denom::{}", &token_b),
                ))?;

            self.denom_map.insert(
                String::from(token_a.clone()),
                vec![denom_map_entry_for(&token_a, "neutron", "osmosis", ctx)
                    .expect("Failed to get denom map entry for untrn")],
            );
            self.denom_map.insert(
                String::from(token_b.clone()),
                vec![denom_map_entry_for(&token_b, "neutron", "osmosis", ctx)
                    .expect("Failed to get denom map entry for untrn")],
            );
            ...
            self.denom_map.insert(
                ...
                vec![denom_map_entry_for(&ibc_denom_a, "osmosis", "neutron", ctx)
                    .expect("Failed to get denom map entry for untrn")],
            );
            self.denom_map.insert(
                String::from(ibc_denom_b.clone()),
                vec![denom_map_entry_for(&ibc_denom_b, "osmosis", "neutron", ctx)
                    .expect("Failed to get denom map entry for untrn")],
            );
